...
def test_a_submission_goes_to_the_manager_and_only_published_files_load(
    ...
    monkeypatch.setenv("SYNAPSE_USER_MANAGER", "Jane Doe")
    monkeypatch.setenv("SYNAPSE_USER_MANAGER_BAND", "42")
    ...
    monkeypatch.setenv("SYNAPSE_USER_MANAGER_BAND", "30")
...
def test_the_skills_page_and_the_popup_carry_the_workflow():
    ...
    for piece in ('"/reviews"', "ReviewSubmit", "ReviewDecision", "ReviewResubmit",
        ...
        assert piece in (REPO / "apps" / "synapse_admin" / "backend" / "chat.py").read_text(
