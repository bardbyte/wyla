...
| what a chat cost (tokens in and out, model calls, wall time, turns) | `sahs/assistant/runtime.py` (`_settle_usage`, a bus sink on the parent turn's `turn_done`; a task's sub-turn counts once through the parent); read by `GET /api/chat/sessions*`, `/api/chat/search`, and per person by `GET /api/admin/users` (`backend/admin.py`, one `SUM` grouped by `OwnerUserId`; under `SAHS_STORE=local` the one developer's totals from the local store's `usage_totals`) | `SpannerAssistantStore.add_usage` (one DML statement, atomic), `set_message_usage` (the same usage onto the turn's final message), `usage_by_owner` | `ChatSessions.TokensIn`, `TokensOut`, `ModelCalls`, `ElapsedMs`, `Turns` (`009_usage.sql`); `ChatMessages.Payload` gains `usage` on the turn's final assistant message, for the footer the chat replays |
