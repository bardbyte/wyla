...
**P2-3 · Sensitive MDM surfaces must never be ingested — ✅ NOW ENFORCED IN CODE.** `ApiPollingInfo` (`/api-polling-info*`) carries endpoint tokens, header tokens, `uid`, `pwd`. The full crawler (`synapse/loaders/mdm_crawler.py`) gates every request through a path allowlist (`/datasets/`, `/app-flows/`, `/portal-v2/`, `/table-lineages`, `/attribute-lineage`, `/lifecycle`) AND a hard deny pattern (`api-polling-info|/keys|key-schema-mappings`); GETs only; denials are tested (`test_sensitive_endpoints_are_hard_denied`).
