...
The output is a structured tree showing:
  - every top-level key
  - for each nested object, its keys + types + populated/null status
  - for each list, the keys of the first item
  - sample values for populated fields (truncated to keep output sane)
...
def _print_dict_keys(d: dict[str, Any], indent: int = 2) -> None:
    pad = " " * indent
    if not d:
...
def explore_payload(table_name: str, payload: Any) -> None:
    ...
    _print_dict_keys(data, indent=4)
    ...
    if remaining:
        _print_section("Other top-level keys (not in our digest yet)")
        _print_dict_keys(remaining, indent=4)
