...
**Shipped (round 3 — MDM as the metadata spine):** full read-side MDM crawler (`synapse/loaders/mdm_crawler.py`: exists → schema → ownership → appflow → pipeline governance → table lineage both directions → attribute lineage → lifecycle; GETs only behind a path allowlist with the credential-bearing endpoints hard-denied; per-endpoint raw cache so the whole crawl replays offline and laptop runs resume) · unified digest (`mdm_digest.py`, handles both sensitivity payload shapes, captures `dataset_parent_id` — the spine ID) · **authoritative `business_unit` → `company_domain`** ("what does Risk own?" is now a grounded graph filter) · lifecycle + pipeline-governance facts on Table nodes and in `inspect_table` governance · `DERIVES_FROM` column lineage with derivation logic · **table-identity normalization** in `canonical_uri` (skills' `common.roll_rate_calc`, gold-SQL's FQN, and MDM's bare name now fuse onto ONE node — multi-witness tiers finally compound) · `pipeline.py --mdm-crawl` + `BQ_API_ENDPOINT` PSC override for the SDK client.

**Shipped (round 2 — execution + presentation):** MDM `data_category`/`data_sub_category` → domain axes on Table nodes · **gated warehouse execution** (`synapse/warehouse/runner.py`: read-only shape gate → graph-guardrail gate → live BigQuery dry-run → byte/cost budget → row-capped execute, audit ledger on every attempt; SVC-ID execution confirmed on the laptop) · **visualization layer** (`synapse/viz/`: typed chart specs — stat/line/bar/top-N/table/dashboard — rendered to themed dependency-free HTML with a CVD-validated palette in both modes; discipline enforced as validators, not taste) · **agent craft skills** (`apps/analyst/skills/`: response-design, visualization, executive-communication — SKILL.md format, progressively disclosed via `list_agent_skills`/`load_agent_skill`) so the analyst picks the right form per question shape × audience (analyst / VP / C-suite).
