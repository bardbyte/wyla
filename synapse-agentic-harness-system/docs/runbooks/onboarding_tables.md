...
python scripts/pipeline.py build-graph --graph graph \
...
python scripts/pipeline.py compile --graph graph --builds builds \
  --out graph/runs/onboard_<unit> --plain
...
| counter | must read |
|---|---|
| `ledger.inventoried` | **0** — a file under an input root that nothing read |
| `lob_map.shared_memberships` | **6** |
| `lob_map.shared_without_home` | **absent** — a shared row whose table no row anywhere calls home |
| `lob_map.multiple_homes` | **absent** — two units both claiming to own one table |
| `compile.tables` | up by 12 |
| `⚠ regressions` | empty |

`expressions.skipped_unresolvable_table` and
`value_meanings.skipped_unknown_table` are REPORTED, not gated: both
name tables from across the warehouse that a 46-row crosswalk was
never meant to hold. A crosswalk row that is genuinely missing does
not show up there — it **blocks the build** at the crosswalk gate,
which is why the rows come first.

Then read the table card for one of the six shared tables. Its
line-of-business line should name both memberships and which is which:

```
- line of business: GMNS: … (home; steward) · CFR: … (shared; steward)
```

And in the cosmos, each shared body sits in its **home** well with a
tether to the well that shares it.
...
|---|---|
| table card `- line of business:` | `GMNS: … (home; steward) · <UNIT>: … (shared; steward)` — the role is named only when a table has more than one membership, because on a single-LOB table "home" says nothing the reader needs |
| owning LOB | unchanged — a share does not dilute ownership |
| `indexes/lob.jsonl` | `table_roles` and `shared_tables` per unit |
| Cosmos | the body sits in its **home** well rather than the first mapped one, and each membership tether carries `role: home` / `role: shared` |
| build report | `shared_memberships`, plus `shared_without_home` and `multiple_homes` when the map is ambiguous |
