...
def _gate_counters(run_manifest: Any, build_manifest: Any
    ...
    # a table that does not resolve to a crosswalk identity contributes
    # nothing, silently. Only report the counters a loader actually
    # emits — inventing a zero for one that is never incremented would
    # read as "no skips" when the truth is "nobody counted"
    for source in ("std_tech", "bq", "expressions"):
        rep = reports.get(source)
        if isinstance(rep, dict) and "skipped_unresolvable_table" in rep:
            out[f"{source}.skipped_unresolvable_table"] = \
                rep["skipped_unresolvable_table"]
    meanings = reports.get("value_meanings") or {}
    for key in ("skipped_unknown_table", "skipped_unknown_column"):
        if key in meanings:
            out[f"value_meanings.{key}"] = meanings[key]
    ...
    for key in ("tables", "metrics", "coverage_unaccounted"):
        if key in counts:
            out[f"compile.{key}"] = counts[key]
    return out
...
def main(argv: list[str] | None = None) -> int:
    ...
    cmd = [sys.executable, str(SILO / "scripts" / "pipeline.py"),
    ...
    # 2. compile
    rc = _run([sys.executable, str(SILO / "scripts" / "pipeline.py"),
    ...
    for key, value in gates.items():
        ...
        # GATED: a file under an input root that nothing read, and an
        # ARCHIVE or CATALOG entry naming a table the crosswalk does
        # not carry — both are onboarding failures.
        # REPORTED, never gated: expressions and the value-meaning
        # index name tables from across the warehouse, most of which
        # this graph is not meant to hold; a non-zero there is the
        # normal shape of a 46-table crosswalk over a large estate.
