...
GoogleSQL DDL files, applied in order, one database.
...
| `004_google_oauth.sql`, `006_external_identities.sql` | the Google connection (user-delegated BigQuery), the Okta identities and the sign-in states | nothing on the laptop |
...
for `001`, `004` and `006`; `SpannerAssistantStore`
...
All the files are applied; the first rollout writes to the identity
