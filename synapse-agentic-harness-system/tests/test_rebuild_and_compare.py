...
def test_adding_a_table_shows_up_as_exactly_that(tmp_path: Path):
    ...
    # the ledger has no unread file, and the table count grew by one
    assert gates["ledger.inventoried"] == 0
    assert gates["compile.tables"] == 3
    # expressions and the value index name tables from across the
    # warehouse that this crosswalk is not meant to hold — reported,
    # never gated, or every real run would fail on its own normal shape
    assert gates["expressions.skipped_unresolvable_table"] >= 0
    # the steward map is clean: no shared row without a home row, no
    # table claimed home by two units
    assert gates["lob_map.shared_memberships"] == 1
    assert "lob_map.shared_without_home" not in gates
    assert "lob_map.multiple_homes" not in gates
    # the value-meaning index is warehouse-wide and names a table this
    # graph does not hold — reported, never gated
    assert gates["value_meanings.skipped_unknown_table"] >= 1
