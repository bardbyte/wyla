...
the card, the unit shelf and the cosmos can all say "shared from GMNS"
instead of flattening two different claims into one.
...
class LobRow(BaseModel):
    ...
    aliases: list[str] = []
    # home: this LOB owns the table (default). shared: this LOB uses a
    # table another LOB owns. Anything else refuses to load — a third
    # word would be a new claim nobody has defined.
    role: str = "home"
