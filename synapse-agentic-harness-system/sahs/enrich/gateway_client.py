"""Gemini through the gateway, as the harness's model client.
...
ONE burst per call, because the gateway serves ``generateContent`` and not
...
class GatewayClient:
    ...
    def _post(self, body: dict[str, Any], *,
        ...
        """The body to the gateway → Gemini's answer as a dict. A dead token is
        ...
        for attempt, backoff in enumerate(_BACKOFFS + (None,)):
            ...
            if status == 200:
                ...
                if not isinstance(payload, dict):
                    raise EnrichTransportError(
                        "the gateway answered 200 with a body that is not JSON")
                return payload
            ...
            if status in _RETRY_STATUSES and backoff is not None:
                ...
                self.sleep(backoff)
                continue
            raise EnrichTransportError(f"the gateway refused: {reason}")
        raise EnrichTransportError(
            f"the gateway unreachable after {len(_BACKOFFS) + 1} attempts — "
            f"last: {last}")
