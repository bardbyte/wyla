...
def http_call(route: Route, method: str, url: str,
    ...
    not an exception; only a transport failure raises GatewayError."""
    ...
    try:
        ...
        raise GatewayError(f"unreachable via {redact_url(str(e.reason))}") from e
    except (TimeoutError, OSError) as e:
        raise GatewayError(f"transport: {type(e).__name__}: {e}") from e
...
def model_plane(env: dict[str, str] | None = None) -> str:
    """Which plane the chat's model calls ride: ``vertex`` or ``gateway``.
    SAHS_MODEL_PLANE names one; ``auto`` (the default) picks the gateway when
    ...
    return "gateway" if has_gateway else "vertex"
...
def thinking_budgets(env: dict[str, str] | None = None) -> dict[str, int]:
    ...
    if (env.get("GATEWAY_JSON_THINKING_BUDGET") or "").strip().isdigit():
        out["json"] = int(env["GATEWAY_JSON_THINKING_BUDGET"].strip())
    return out
...
class TokenManager:
    """One the identity service token at a time: minted on demand, reused while
    ...
    def _mint(self) -> None:
        if not self.cfg.app_id or not self.cfg.secret:
            raise GatewayError("APP_ID and APP_SECRET are needed to mint a "
                           "The identity service token (or AUTH_MODE=env with "
                           "GEMINI_BEARER_TOKEN)")
        ...
        if status != 200:
            raise GatewayError("The identity service refused the token request: "
                           + _error_text(status, raw, answer_headers))
        token, _field = extract_token(_json(raw))
        if not token:
            raise GatewayError("The identity service answered 200 but no token field "
                           "was recognized")
        self._adopt(token)
        self.mints += 1
