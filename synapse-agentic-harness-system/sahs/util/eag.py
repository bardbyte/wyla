...
def vertex_configured(env: dict[str, str] | None = None) -> bool:
    ...
    exists (LUMI_VERTEX_SA_KEY or GOOGLE_APPLICATION_CREDENTIALS) and
    a project."""
    env = dict(os.environ if env is None else env)
    key = (env.get("LUMI_VERTEX_SA_KEY")
           or env.get("GOOGLE_APPLICATION_CREDENTIALS") or "").strip()
    project = (env.get("VERTEX_PROJECT_ID")
               or env.get("LUMI_VERTEX_PROJECT") or "").strip()
...
def model_plane(env: dict[str, str] | None = None) -> str:
    ...
    in the environment and falls back to EAG only when EAG alone is
    configured. A chat can switch from the composer; this is the
    plane a new chat starts on."""
    env = dict(os.environ if env is None else env)
    wanted = (env.get(PLANE_VAR) or "auto").strip().lower()
    if wanted in ("vertex", "eag"):
        return wanted
    if eag_configured(env) and not vertex_configured(env):
        return "eag"
    return "vertex"
...
def plane_note(env: dict[str, str] | None = None) -> str:
    ...
    has_eag = eag_configured(env)
    has_vertex = vertex_configured(env)
    if has_vertex:
        return f"{PLANE_VAR} unset: the Vertex contract is present"
    if has_eag:
        return (f"{PLANE_VAR} unset: EAG credentials present, no Vertex "
                "key")
    return f"{PLANE_VAR} unset: no Vertex key and no EAG credentials"
